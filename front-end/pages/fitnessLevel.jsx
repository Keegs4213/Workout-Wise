...
function FitnessLevelPage() {
  ...
  const handleLevelClick = async (level) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3245/users/update/${userId}`, {
        fitnessGoal,
        fitnessLevel: level.name
      });
  
      // Save workout and nutrition plans to local storage
      localStorage.setItem("workoutPlan", JSON.stringify(response.data.workoutPlan));
      localStorage.setItem("nutritionPlan", JSON.stringify(response.data.nutritionPlan));
